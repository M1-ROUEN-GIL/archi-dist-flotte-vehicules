...
Le projet a migre vers une architecture **Full Helm**. Plutot que de separer les manifests des microservices de la configuration de l'infrastructure, nous avons cree deux Charts principaux :

\begin{itemize}
  \item \textbf{Infrastructure lourde} (\texttt{fleet-infra}) : PostgreSQL, Redis via Bitnami, Kafka (manifest KRaft), et toute la stack d'observabilite LGTMe (Loki, Grafana, Tempo, Prometheus).
  \item \textbf{Application} (\texttt{fleet-app}) : Contient uniquement nos microservices metiers et les regles d'Ingress.
\end{itemize}

L'observabilite est integree comme un \textbf{sub-chart} dans \texttt{fleet-infra}. Cette approche permet de disposer d'une \textbf{source de verite unique} pour les fichiers de configuration (Prometheus, OTel Collector) qui sont partages entre Docker Compose (via volumes) et Kubernetes (via ConfigMaps generes par Helm).

Cela permet a n'importe quel developpeur de lancer toute l'infrastructure en \textbf{2 commandes seulement} :

\begin{lstlisting}[language=bash, caption=Lancement complet de l'infrastructure]
# 1. Installer l'infra (DBs, Kafka, Observabilite)
helm upgrade --install fleet-infra ./infra/helm/fleet-infra/ -n flotte-namespace
# 2. Installer les microservices applicatifs
...
Les fichiers de configuration (OTel Collector et Prometheus) sont centralises dans le dossier \texttt{infra/helm/fleet-infra/charts/observability/config/}. Ces fichiers servent a la fois pour les montages de volume Docker Compose et pour les ConfigMaps Kubernetes via la fonction \texttt{.Files.Get} de Helm.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    OTel Collector       & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
    Jaeger / Prometheus  & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
    Grafana / Loki       & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
...
|-- infra/
|   |-- helm/
|   |   |-- fleet-infra/            # Chart INFRA (DBs, Kafka, Obs)
|   |   |   |-- values.yaml         # Config de toute l'infra
|   |   |   |-- templates/          # Kafka, Postgres (si custom)
|   |   |   `-- charts/
|   |   |       `-- observability/  # Sub-chart (Loki, Grafana, etc.)
