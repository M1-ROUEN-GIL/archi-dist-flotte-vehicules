...
% ------------------------------------------------------------
\newpage
\section{Instrumentation du code pour l'Observabilité}

\begin{infobox}
\textbf{Objectif :} Garantir une visibilité complète sur l'état de santé et les performances du microservice en production via les trois piliers de l'observabilité : traces, métriques et logs.
\end{infobox}

\vspace{0.5cm}

Faisant suite à l'installation de la stack d'observabilité (Loki, Grafana, Jaeger) la semaine précédente, cette semaine a été consacrée à l'instrumentation du code du microservice Véhicules. Le service a été nativement instrumenté en utilisant le standard industriel \textbf{OpenTelemetry}. Cette approche permet de suivre le cycle de vie d'une requête à travers l'ensemble du système distribué.

\subsection{Collecte des données via OpenTelemetry SDK}

L'instrumentation repose sur l'intégration de \textbf{Micrometer Tracing} et de l'exportateur \textbf{OTLP} (OpenTelemetry Protocol). Le service pousse ses données vers un collecteur centralisé qui les redistribue ensuite vers les outils spécialisés de notre stack :

\begin{itemize}
    \item \textbf{Traces (Jaeger)} : Chaque requête entrante génère un \texttt{Trace ID} unique. Cela permet de visualiser le temps passé dans chaque couche (Contrôleur, Service, Base de données) et de diagnostiquer les goulots d'étranglement.
    \item \textbf{Métriques (Prometheus / Grafana)} : Le service expose un endpoint \texttt{/actuator/prometheus}. Nous collectons des métriques JVM (mémoire, CPU) ainsi que des indicateurs métier (nombre de véhicules créés, taux d'erreur HTTP).
    \item \textbf{Logs structurés (Loki)} : Les logs ne sont plus de simples fichiers textes mais sont envoyés de manière structurée, facilitant la corrélation avec les traces grâce au \texttt{Trace ID} injecté automatiquement dans chaque ligne de log.
\end{itemize}

\subsection{Configuration de l'exportation}

La configuration est centralisée dans le fichier \texttt{application.yaml}, permettant de basculer facilement entre un environnement de développement local et le cluster Kubernetes.

\begin{lstlisting}[caption=Configuration de l'exportateur OTLP dans Spring Boot]
management:
  tracing:
    sampling:
      probability: 1.0 # Capture de toutes les traces
  otlp:
    tracing:
...
Cette instrumentation précoce garantit que nous pourrons monitorer l'impact des futurs services (comme la localisation en temps réel) sur les performances globales de l'application.
