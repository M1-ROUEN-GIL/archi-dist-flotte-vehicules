...
\subsection{Gestion des redirections Ingress et réécriture d'URL}
L'utilisation d'une annotation globale \texttt{rewrite-target} provoquait une boucle de redirection infinie sur l'interface d'administration de Keycloak. En effet, Keycloak redirigeait la racine vers \texttt{/admin/}, et l'Ingress réécrivait cette URL en \texttt{/}, créant un cycle sans fin.

\textbf{Solution :} Scinder l'Ingress en deux ressources distinctes : une pour les APIs microservices avec réécriture via des \textit{regex capture groups}, et une pour Keycloak sans réécriture, préservant ainsi l'intégrité des chemins profonds du SSO.

\subsection{Synchronisation des secrets de base de données}
Un conflit d'authentification est survenu entre Keycloak et PostgreSQL suite à l'utilisation de mots de passe générés aléatoirement par Helm lors du premier déploiement. Bien que des secrets aient été créés, ils n'étaient pas alignés avec l'état réel de la base de données après un redémarrage partiel.

\textbf{Solution :} Harmonisation des identifiants en forçant l'utilisation du fichier \texttt{values.secret.yaml} lors de l'installation Helm et mise à jour de la configuration Keycloak pour utiliser l'utilisateur \texttt{postgres}, garantissant une connexion stable et prédictible dès le premier démarrage.
