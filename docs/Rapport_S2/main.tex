...
vers le registre d'images.
\end{infobox}

\vspace{0.3cm}

Le projet utilise \textbf{GitHub Actions} pour orchestrer le cycle de vie du
developpement. Le workflow est defini dans \texttt{.github/workflows/ci-cd.yml}
et se decompose en quatre phases majeures :

\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
  \item \textbf{Tests des microservices Java :} Utilisation d'une matrice pour tester
        en parallele les services (\texttt{driver}, \texttt{events}, \texttt{maintenance},
        \texttt{vehicle}) via Maven et JDK 21.
  \item \textbf{Tests du microservice Node.js :} Pipeline specifique pour le
        \texttt{location-service} avec installation propre des dependances (\texttt{npm ci}).
  \item \textbf{Validation de l'infrastructure (Linting) :} Verification syntaxique
        des charts Helm pour prevenir toute erreur de configuration Kubernetes.
  \item \textbf{Build \& Push Docker :} Phase executee uniquement sur la branche
        \texttt{main}. Les images sont poussees vers \textbf{GHCR (GitHub Container Registry)}
        avec un double tagging (\texttt{latest} et \texttt{SHA du commit}).
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Automatisation :} Maintenir le pipeline GitHub Actions (tests unitaires
        et linting a chaque Pull Request)
